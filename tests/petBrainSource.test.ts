...
describe("pet brain model binding source", () => {
  it("binds each visible pet to a model while sharing the active conversation", () => {
    ...
    expect(appSource).toContain("sendChat(requestMessages, currentPetModelId)");
    expect(appSource).toContain("runAgentTask(content.trim(), currentPetModelId)");
    expect(appSource).toContain("sendChat(requestMessages, activeAgentModelId)");
    expect(appSource).toContain("runAgentTask(content.trim(), activeAgentModelId)");
    ...
    expect(globalSource).toContain("sendChat(messages: ChatMessage[], modelId?: string)");
    expect(globalSource).toContain("runAgentTask(task: string, modelId?: string)");
  });
});
